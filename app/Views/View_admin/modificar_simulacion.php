...
<body>
    <div class="container mx-auto mt-10">
        <h1 class="text-4xl font-bold text-center mb-8">MODIFICAR SIMULACIONES</h1>

        <div class="flex justify-center items-center">
            <div class="w-1/3">
                <div>
                    <label for="simulacion_select" class="block text-sm font-medium text-gray-700">Seleccionar Simulación</label>
                    <select id="simulacion_select" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" onchange="cargarSimulacion(this)">
                        <option value="">Seleccione una simulación</option>
                        <?php foreach ($simulaciones as $simulacion) : ?>
                            <?php
                            $nombreProducto = '';
                            $imagenProducto = ''; // Variable para almacenar la imagen del producto
                            foreach ($productos as $producto) {
                                if ($producto['id_productos'] == $simulacion['id_productos']) {
                                    $nombreProducto = $producto['nombre_producto'];
                                    $imagenProducto = $producto['imagen_producto']; // Asumiendo que la ruta de la imagen está aquí
                                    break;
                                }
                            }
                            ?>
                            <option value="<?= $simulacion['id_simulaciones'] ?>" data-file="<?= $simulacion['archivo_simulacion'] ?>" data-nombre="<?= $nombreProducto ?>" data-imagen="<?= $imagenProducto ?>">
                                Producto ID: <?= $simulacion['id_productos'] ?> - <?= $nombreProducto ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>

                <form action="<?= base_url('modificar_simulacion') ?>" method="post" enctype="multipart/form-data" class="space-y-6">
                <?= csrf_field() ?>
                    <input type="hidden" id="simulacion_id" name="simulacion_id">

                    <div>
                        <label for="productModel" class="block text-sm font-medium text-gray-700">Subir modelo 3D (.glb)</label>
                        <input type="file" id="productModel" name="modelo_producto" accept=".glb" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <span id="fileDisplay" class="mt-2 text-gray-600">No se ha seleccionado ningún archivo.</span>
                    </div>
                    ...
                    <div>
                    <button type="button" onclick="confirmDelete()" class="w-full bg-red-600 text-white py-2 px-4 rounded-md hover:bg-red-500">Eliminar Simulación</button>
                    </div>
                </form>
            </div>
    ...
    <script>
    function cargarSimulacion(selectElement) {
        var selectedOption = selectElement.options[selectElement.selectedIndex];
        var idSimulacion = selectedOption.value; // ID de la simulación seleccionada
        var nombreProducto = selectedOption.getAttribute("data-nombre");
        var nombreArchivo = selectedOption.getAttribute("data-file");
        var imagenProducto = selectedOption.getAttribute("data-imagen"); // Ruta de la imagen del producto

        // Actualizar el campo oculto con el ID de la simulación
        document.getElementById("simulacion_id").value = idSimulacion;

        // Mostrar el nombre del archivo si existe
        if (nombreArchivo) {
            document.getElementById("fileDisplay").innerText = 'Archivo actual: ' + nombreArchivo;
        } else {
            document.getElementById("fileDisplay").innerText = 'No hay archivo cargado.';
        }

        // Actualizar la imagen de previsualización con la imagen del producto
        if (imagenProducto) {
            // Asegurarse de que la ruta de la imagen sea correcta
            document.getElementById("productPreview").src = `<?= base_url('imagenes_producto/imgs') ?>/${imagenProducto}`;
        } else {
            document.getElementById("productPreview").src = 'https://static.vecteezy.com/system/resources/previews/016/017/372/non_2x/image-upload-free-png.png'; // Imagen por defecto si no hay
        }
    }
    </script>
...
function confirmDelete() {
    const simulacionId = document.getElementById('simulacion_select').value; // Cambia el ID de 'producto_select' a 'simulacion_select'
    
    // Verificar si se ha seleccionado una simulación
    if (simulacionId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = '<?= base_url('eliminar_simulacion') ?>'; // Asegúrate de que esta ruta sea correcta
                
                const hiddenInput = document.createElement('input');
                hiddenInput.type = 'hidden';
                hiddenInput.name = 'simulacion_id'; // Cambia el nombre a 'simulacion_id'
                hiddenInput.value = simulacionId;
        ...
        Swal.fire({
            icon: 'error',
            title: '¡Error!',
            text: 'Por favor, selecciona una simulación antes de intentar eliminar.',
